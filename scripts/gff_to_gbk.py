...
Usage:
    gff_to_genbank.py <GFF annotation file> [<FASTA sequence file> <molecule type> <output file>]
...
def main(gff_file, genbank_file, fasta_file=None, molecule_type="DNA"):
    if fasta_file:
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or len(sys.argv) > 5:
        print("Usage: python gff_to_gbk.py <GFF annotation file> <output file> [<FASTA sequence file> <molecule type>]")
        sys.exit(1)
    
    gff_file = sys.argv[1]
    genbank_file = sys.argv[2]
    fasta_file = sys.argv[3] if len(sys.argv) > 3 else None
    molecule_type = sys.argv[4] if len(sys.argv) > 4 else "DNA"
    main(gff_file, genbank_file, fasta_file, molecule_type)